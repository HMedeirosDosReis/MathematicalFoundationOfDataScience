...
---
To run:
must have run testing.R with all 65 people
must have run demographics.Rmd

```{r}
#options(max.print=10000)
#var
var2 <- as.data.frame.matrix(var)
#var2
colnames(var2) <- folders
rownames(var2) <- folders
var2

```

How to read this:
So it isn't for say not in numerical order
because I checked both y and prediction which is what makes this matrix and they are in the correct order

So how to read this beast
so if var2_ii is not 20 then check the full column where var2_ii != var2_ii and var2_ii != 0. Here will be var2_ki (row changes but column doesnt) and that k/row number will tell you who it is incorrectly predicting var2_ii as 



```{r}
numberofpics <- 20 #the max number of pictures of an individual in data set
...
colnames(incorrectresults)[4] <- "Accuracy of individual (%)"
colnames(incorrectresults)[5] <- "Algorithm predicted individual incorrectly as..."
incorrectresults
```
...
paste0("Total Accuracy of the algorithm ", round(100*sum/(numberofpics*length(folders)), 2), "%")
...
paste0(round(100*sum(results$gender == "M")/sum(demographics$gender == "M"),2),"% of the men were detected inaccurately") # (12/52)
paste0(round(100*sum(results$gender == "F")/sum(demographics$gender == "F"),2), "% of the women were detected inaccurately") #(4/13)

```

#Further Observations that required some manual interpretation

Naomi is hard to find a reason to why she is being a problem/is the prediction of other people

- looking at her data set nothing seems off and the genders or ethnicity do not even match to those that are getting predicted as her

- looking at the photo josh was predicted as Naomi, I just don't see how the computer would do it

Off the bat no significant between our pictures and the other individuals
